...
try:
    ...
    print("[Consumer1] Підключено до MinIO")
except Exception as e:
    print(f"[Consumer1] Помилка підключення до MinIO: {e}")
    exit(1)
...
def setup_consumer():
    try:
        ...
        print(f'[Consumer1] Помилка підключення до Kafka: {e}')
        return None

consumer = setup_consumer()
while not consumer:
    print('[Consumer1] Повторна спроба підключення...')
    time.sleep(5)
    consumer = setup_consumer()

print('[Consumer1] Підключено до Kafka. Обробка повідомлень...')
...
try:
    for message in consumer:
        try:
            ...
            if 'start_time' not in data:
                print("[Consumer1] Поле 'start_time' відсутнє")
                continue
            ...
            if month_year != current_month_year:
                if current_file:
                    try:
                        ...
                        print(f"[Consumer1] Файл {current_file} завантажено (зміна місяця)")
                        os.remove(current_file)
                    except Exception as e:
                        print(f"[Consumer1] Помилка: {e}")
            ...
            with open(current_file, 'a', newline='') as f:
                ...
                current_row_count += 1
                print(f"[Consumer1] Записано до {current_file}: {data}")

                # Завантажити частину файлу
                if current_row_count >= MAX_ROWS_PER_FILE:
                    try:
                        minio_client.fput_object("default", os.path.basename(current_file), current_file)
                        print(f"[Consumer1] Файл {current_file} завантажено (частина)")
                        current_row_count = 0
                    except Exception as e:
                        print(f"[Consumer1] Помилка: {e}")

        except Exception as e:
            print(f'[Consumer1] Помилка обробки: {e}')

except KeyboardInterrupt:
    print("[Consumer1] Зупинено користувачем")
    ...
    if current_file and os.path.exists(current_file):
        try:
            minio_client.fput_object("default", os.path.basename(current_file), current_file)
            print(f"[Consumer1] Фінальний файл {current_file} завантажено")
            os.remove(current_file)
        except Exception as e:
            print(f"[Consumer1] Помилка: {e}")